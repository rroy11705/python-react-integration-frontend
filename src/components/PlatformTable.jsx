...
import { useFetchPlatforms, useRemovePlatform } from '../api/queries/platforms.queries';
import CreateUpdatePlatformForm from './CreateUpdatePlatformForm';
...
const Actions = ({ row }) => {
  ...
  const handleRemove = () => {
    removePlatform.mutate(row.original.id)
  };
  return (
    <div>
      <CreateUpdatePlatformForm data={row.original} modalId={`update-platform-${row.original.id}`} />
      <button style={{ background: '#428bca' }} data-modal-target={`update-platform-${row.original.id}`}>
...
const PlatformTable = () => {
  ...
  const defaultColumns = useMemo(
    () => [
      ...
      columnHelper.accessor(row => row.website, { 
        ...
        header: 'Website' ,
        cell: props => (
          <a href={props.getValue()} target='_blank' rel="noreferrer" >{props.getValue()}</a>
        )
      }),
  ...
  return (
    <div className='wrapper'>
      <CreateUpdatePlatformForm modalId="create-update-platform" />
      <button data-modal-target="create-update-platform">
        Create Platform
      </button>
